...
class Etf_stats():
        ...
        for symbol, value in self.symbols.items():
            ...
            for currency in info["assets"]:
                if currency["currency"] == symbol.split('_')[0]:
                    xt_coin_reduce = round(value["init_amount"] - float(currency["totalAmount"]), 2)
            ...
            self.symbols[symbol]["xt_coin_reduce_convertusdt"] = xt_coin_reduce_convertusdt
    ...
    def get_position_bn(self):
        ...
        return bn_usdt_remain, bn_unRealizedProfit, bn_cost_usdt, bn_coin_remain, bn_entry_price
...
while True:
    ...
    bn_usdt_remain, bn_unRealizedProfit, bn_cost_usdt, bn_coin_remain, bn_entry_price = etf_stats.get_position_bn()
    ...
    for symbol, value in etf_stats.symbols.items():
        ...
        logging_info += f"({symbol}: {value['xt_coin_reduce']} {value['xt_coin_remain']}) "
