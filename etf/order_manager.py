...
class Order():
    def __init__(self, symbol=None, OrderId=None, side=None, 
                 type="LIMIT", timeInForce="GTC", bizType="SPOT", price=None, quantity=None, 
                 quoteQty=None):
...
class OrderManager:
    ...
    # add orders
    def add_order(self, symbol, side="BUY", type="LIMIT", price=None, quantity=None, is_wash_trading=False):
        order = Order(symbol=symbol, side=side, type=type, price=price, quantity=quantity)
        ...
        # TODO get orderid from response
        
        #if response and not is_wash_trading:
        #    self.sent_orders[response["orderId"]] = {}    
        #     current_time = time.time()
        #     self.open_orders[response["orderId"]] = {
        #         "symbol": order.symbol,
        #         "side": order.side,
        #         "price": order.price,
        #         "quantity": order.quantity,
    ...
    def cancel_all_open_orders(self, symbol=None, biz_type="SPOT", side=None):
        response = self.client.cancel_open_orders(symbol=symbol, biz_type=biz_type, side=side)
        if response:
            self.open_orders = {}
    ...
    def write_orders(self):
        ...
        local_data = [
            ...
            if str(item["orderId"]) not in self.remove_data
        ]
        data_ids = [str(item["orderId"]) for item in data]
        logging.info(f"comparing {len(data_ids)} data in csv")  
        keep_data = [orderid for orderid in self.canceled_orders if orderid not in data_ids] # strange
        ...
        logging.info(f"########### found {len(data)} orders in local, keep {len(local_data)} orders")
        logging.info(f"keep canceled_orders {len(self.canceled_orders)}")
        existing_ids = {str(item["orderId"]) for item in local_data}
        new_data = local_data + [order for order in self.open_orders.values() if str(order["orderId"]) not in existing_ids]
    ...
    def get_position3(self, symbol, currencies):
        ...
        for currency in info["assets"]:
            if currency["currency"] == symbol.split("_")[0].lower():
                ...
                #    self.last_amount_5l = 
                delta_amount = float(currency["totalAmount"]) - self.last_amount # if < 0, user buy, > 0, user sell
    ...
    def get_position2(self, symbol):
        ...
        for t in range(0, len(self.sent_orders), max_batch_size):
            chunked_get_orders.append([order_id for order_id in filtered_sent_orders_ids[t: t + max_batch_size]])
            ...
            if response:
                for res in response:
                    if res["state"] == 'CANCELED':
                        ...
                        logging.info(datetime.fromtimestamp(updatedTime, tz=timezone.utc).astimezone(timezone(timedelta(hours=8))))
                        logging.info(f"PARTIALLY_FILLED: {res}")
                        
                        if res["orderId"] in self.partially_filled_orders:
                            last_leavingQty = self.partially_filled_orders[res["orderId"]]["leavingQty"]
                        ...
                        logging.info(f"deltaQty {deltaQty}, * price {res['price']}, delta position {deltaQty * float(res['price'])}")
        ...
        delta_position += self.exposure["amount"] * mid_price # exposure only affect the delta_position
        logging.info(f"cmu_deltaQty {cmu_deltaQty}, remain amount {self.amount} (exposure: {self.exposure['amount']}), price {mid_price}, delta position {delta_position}, remain position {self.position}")        
    ...
    def get_position(self, symbol):
        ...
        logging.info(f"find {len(order_ids)} in memory and {len(pre_order_ids)} in local")
        #order_ids += pre_order_ids
        
        # remove canceled orders
        order_ids = [order_id for order_id in order_ids if order_id not in self.canceled_orders]
        #logging.info()
        if len(order_ids) == 0:
            return 0
        ...
        for t in range(0, len(order_ids), max_batch_size):
            chunked_get_orders.append([order_id for order_id in order_ids[t: t + max_batch_size]])
