...
# from balance_thread import get_balances
...
class Spot:
  ...
  "items": [
    ...
    def get_history_orders(self, symbol=None, biz_type="SPOT", side=None, type="LIMIT", order_id=None, from_id=None,
                           direction=None,
        ...
        """
        vars = locals()
        #params = {"symbol": symbol, "bizType": biz_type, "side": side, "type": type, "orderId": order_id,
        #          "fromId": from_id, "direction": direction, "limit": limit, "startTime": start_time, "endTime": end_time,
        #          "hiddenCanceled": hidden_canceled}
    ...
    def withdraw(self, currency, chain, amount, address):
        ...
        currency 和 chain 参考上面的返回 amount
        {"currency":"xt","supportChains":[{"chain":"XT Smart Chain","depositEnabled":true,"withdrawEnabled":true,"withdrawFeeAmount":1,"withdrawMinAmount":5,"depositFeeRate":0}]}
        :param currency:
        :param chain:
        :param amount:
        :param address:
...
class RandomWithdraw:
    address_list = []
    address_list_finished = []
    currency = "xt"
    chain = "XT Smart Chain"
    all_amount = 3000
    ...
    def __init__(self, access_key, secret_key):
        self.count = 0
        self.xt_spot = Spot(host="https://sapi.xt.com",
                            access_key=access_key,
                            secret_key=secret_key)
        self.load_address()

    def record_finished_address(self, address, amount):
        file_path = "record.txt"
        with open(file_path, 'a' if os.path.exists(file_path) else 'w') as file:
            file.write(address + " " + amount + "\n")
            print(f"写入已转地址:{address}")

    def load_address(self):
        file_path = "xsci_1000.txt"
        with open(file_path, 'r') as file:
            content = file.read()
        self.address_list = content.splitlines()

    def distribute(self):
        while self.all_amount > 0:
            addr_selected = self.address_list[0]  # 取第一个地址
            amount = round(random.uniform(1, 3), 1)  # 随机分配1-3个
            try:
                res = self.xt_spot.withdraw(currency=self.currency, chain=self.chain, amount=amount,
                ...
                self.record_finished_address(address=addr_selected)
                self.address_list.remove(addr_selected)  # 选择后移除
                self.all_amount -= amount

            self.count += 1
            print(f"{datetime.now()} {self.count} [随机分配地址:{addr_selected},数量:{amount},res:{res}]")
            random_sleep_time = round(random.uniform(5, 23), 1)  # 4*60*60/1000

            if len(self.address_list) == 0:
                print(f"地址全部分配结束,剩余数量:{self.all_amount}")
                break

            print(f"剩余地址数量:{len(self.address_list)},剩余xt数量:{self.all_amount}")
            time.sleep(random_sleep_time)

# netvalue = None
# def calculate_netvalue(nv):
#     global netvalue
#     nv.run()
#     nv.run()
#     re
