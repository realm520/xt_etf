...
class MarketMaker():
    ...
    def make_orders(self, symbol=None, clientOrderId="16559590087220001", netvalue=1.0, env="qa", ordermanager=None):
        if env == "qa":
            # make fake orders first
            fake_batch_order_bid, fake_batch_order_ask = get_orderbook(mid_price=netvalue)
    ...
    def place_orders(self, config, symbol="btc5l_usdt", clientOrderId="16559590087220001", env="qa", ordermanager=None, currencies=None, prec=4):

        netvalue = float(self.r.get(config["netvalue"]).decode())
        
        # netvalue = float(open(config["netvalue"], 'r', encoding='utf8').read().strip())
        ...
        if netvalue:
            ...
            batch_order_bid, batch_order_ask = get_orderbook(mid_price=netvalue, bid_ask_spread=config["bid_ask_spread"])

            goal_orders = batch_order_ask + batch_order_bid
            self.best_sell = batch_order_ask[0]["price"]
            self.best_buy = batch_order_bid[0]["price"]
            logging.info(f"goal ask1: {batch_order_ask[0]} goal bid1: {batch_order_bid[0]}")
            logging.info(f"goal ask-1: {batch_order_ask[-1]} goal bid-1: {batch_order_bid[-1]}")
            ...
            for goal in goal_orders:
                ...
                valid_market_orders = [
                    ...
                    order for order in current_orders if (goal_min_price <= float(order["price"]) <= goal_max_price)
                ]

                total_market_amount = sum(round(float(order["origQty"])) for order in valid_market_orders)
                ...
                # cancle orders
                    ...
                    for order in valid_market_orders:
                        if excess_amount <= 0:
                            break
                        cancel_amount = min(round(float(order["origQty"])), excess_amount)
            ...
            for market_order in current_orders:
                ...
                if ((float(market_order["price"]) < batch_order_ask[0]["price"] and market_order["side"] == "SELL") or (float(market_order["price"]) > batch_order_bid[0]["price"] and market_order["side"] == "BUY")):  
                    logging.info(market_order["price"])
                    cancel_orders.append(market_order)
        
            logging.info(f"[{time.strftime('%H:%M:%S')}] Placing orders based on net value: {netvalue}")
            logging.info(f"[{time.strftime('%H:%M:%S')}] add_orders: {len(add_orders)}")
            logging.info(f"[{time.strftime('%H:%M:%S')}] cancel_orders: {len(cancel_orders)}")
            ...
            logging.info(f"anti_pin_amount_sell {anti_pin_amount_sell} anti_pin_amount_buy {anti_pin_amount_buy} ")
            ...
            if len(add_orders) > 0:
                chunked_add_orders = [add_orders[t: t + max_batch_size_send] for t in range(0, len(add_orders), max_batch_size_send)]
            if len(cancel_orders) > 0:
                # chunked_cancel_orders = []
                for t in range(0, len(cancel_orders), max_batch_size_cancel):
                    chunked_cancel_orders.append([order for order in cancel_orders[t: t + max_batch_size_cancel]])
            if len(chunked_add_orders) > 0 and len(chunked_cancel_orders) > 0:
                for add_order, cancel_order in zip(chunked_add_orders, chunked_cancel_orders):
            ...
            operate_orders.extend((add_order, 'add') for add_order in chunked_add_orders[len(chunked_cancel_orders):])
            operate_orders.extend((cancel_order, 'cancel') for cancel_order in chunked_cancel_orders[len(chunked_add_orders):])
