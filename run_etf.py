...
class EtfStrategy():
    ...
    def run(config, risk_controller, wash_controller, market_maker, depth=None):
        ...
        if config["cancel_all_open_orders"]:
            ...
            # STG5S 特有的行为：先取消一次
            if config.get("strategy_name") == "stg5s" or config.get("prefix") == "stg5s":
...
def get_parser():
    ...
    # 新增策略相关参数
    parser.add_argument("--strategy", type=str, choices=["stg3l", "stg3s", "stg5l", "stg5s"], 
    ...
    parser.add_argument("--sleep-interval", type=int, default=None, help="Main loop sleep interval in seconds.")
...
if __name__ == "__main__":
    ...
    config = {
        "env": args.env if args.env != parser.get_default('env') else strategy_config.get('env', args.env),
        "clientOrderId": args.client_order_id if args.client_order_id != parser.get_default('client_order_id') else strategy_config.get('clientOrderId', args.client_order_id),
        "symbol": prefix + "_usdt",
        "apikey": args.apikey if args.apikey != parser.get_default('apikey') else strategy_config.get('apikey', args.apikey),
        "netvalue": "netvalue_" + prefix,
        "bnsymbol": prefix.upper()[:-2] + "USDT" if prefix else strategy_config.get('bnsymbol', 'STGUSDT'),
        "precision": args.precision if args.precision != parser.get_default('precision') else strategy_config.get('precision', args.precision),
        "prec_amount": args.prec_amount if args.prec_amount != parser.get_default('prec_amount') else strategy_config.get('prec_amount', args.prec_amount),
        "Hedging_interval": args.hedging_interval if args.hedging_interval != parser.get_default('hedging_interval') else strategy_config.get('Hedging_interval', args.hedging_interval),
        "washing_interval": args.washing_interval if args.washing_interval != parser.get_default('washing_interval') else strategy_config.get('washing_interval', args.washing_interval),
        "kline_continuity_interval": args.kline_continuity_interval if args.kline_continuity_interval != parser.get_default('kline_continuity_interval') else strategy_config.get('kline_continuity_interval', args.kline_continuity_interval),
        "Enable_risk_controller": args.enable_risk_controller if args.enable_risk_controller != parser.get_default('enable_risk_controller') else strategy_config.get('Enable_risk_controller', args.enable_risk_controller),
        "Enable_wash_trading": args.enable_wash_trading if args.enable_wash_trading != parser.get_default('enable_wash_trading') else strategy_config.get('Enable_wash_trading', args.enable_wash_trading),
        "cancel_all_open_orders": args.cancel_all_open_orders if args.cancel_all_open_orders != parser.get_default('cancel_all_open_orders') else strategy_config.get('cancel_all_open_orders', args.cancel_all_open_orders),
        "pre_make_orders": args.pre_make_orders if args.pre_make_orders != parser.get_default('pre_make_orders') else strategy_config.get('pre_make_orders', args.pre_make_orders),
        "leverage": args.leverage if args.leverage != parser.get_default('leverage') else strategy_config.get('leverage', args.leverage),
        "precision_amount": args.precision_amount if args.precision_amount != parser.get_default('precision_amount') else strategy_config.get('precision_amount', args.precision_amount),
        "precision_price": args.precision_price if args.precision_price != parser.get_default('precision_price') else strategy_config.get('precision_price', args.precision_price),
        "Enable_hedging": args.enable_hedging if args.enable_hedging != parser.get_default('enable_hedging') else strategy_config.get('Enable_hedging', args.enable_hedging),
        "Exit_with_cancel_all_open_orders": args.exit_with_cancel_all_open_orders if args.exit_with_cancel_all_open_orders != parser.get_default('exit_with_cancel_all_open_orders') else strategy_config.get('Exit_with_cancel_all_open_orders', args.exit_with_cancel_all_open_orders),
        "wash": args.wash if args.wash != parser.get_default('wash') else strategy_config.get('wash', args.wash),
        "anti_pin_usdt": args.anti_pin_usdt if args.anti_pin_usdt != parser.get_default('anti_pin_usdt') else strategy_config.get('anti_pin_usdt', args.anti_pin_usdt),
        "anti_pin_rate": args.anti_pin_rate if args.anti_pin_rate != parser.get_default('anti_pin_rate') else strategy_config.get('anti_pin_rate', args.anti_pin_rate),
        "Enable_market_making": args.enable_market_making if args.enable_market_making != parser.get_default('enable_market_making') else strategy_config.get('Enable_market_making', args.enable_market_making),
        "bid_ask_spread": args.bid_ask_spread if args.bid_ask_spread != parser.get_default('bid_ask_spread') else strategy_config.get('bid_ask_spread', args.bid_ask_spread),
        "sleep_interval": args.sleep_interval if args.sleep_interval is not None else strategy_config.get('sleep_interval', 1),
    }
