...
class XtClient:
    def __init__(self, apikey, symbols):
        ...
        self.last_amount_storage = RedisLastAmountStorage(redis_client=self.r)
    ...
    def get_position3(self):
        ...
        for symbol, value in self.symbols.items():
            try:
                mid_price = float(
                    self.r.get("netvalue_" + symbol.split("_")[0]).decode()
                )
            ...
            for currency in info["assets"]:
                if currency["currency"] == symbol.split("_")[0]:
                    if value["last_amount"] > 0:
                        delta_amount = (
                            float(currency["totalAmount"]) - value["last_amount"]
                        )  # if < 0, user buy, > 0, user sell
                    else:
                        # 尝试从Redis获取last_amount
                        try:
                            stored_amount = self.last_amount_storage.get_last_amount(
                                symbol
                            )
                            if stored_amount is not None:
                                value["last_amount"] = stored_amount
                                delta_amount = (
                                    float(currency["totalAmount"])
                                    - value["last_amount"]
                                )
                                logging.debug(
                                    f"Retrieved last_amount for {symbol} from Redis: {stored_amount}"
                                )
                            else:
                                # Redis中没有数据，首次运行
                                current_amount = float(currency["totalAmount"])
                                value["last_amount"] = current_amount
                                delta_amount = 0
                                logging.info(
                                    f"First run for {symbol}, initialized with current amount: {current_amount}"
                                )
                                # 存储初始值到Redis
                                try:
                                    self.last_amount_storage.set_last_amount(
                                        symbol, current_amount, source="initial"
                                    )
                                except Exception as e:
                                    logging.error(
                                        f"Failed to store initial amount to Redis: {e}"
                                    )
                        except Exception as redis_error:
                            logging.error(f"Redis error for {symbol}: {redis_error}")
                            # Redis完全不可用，使用当前余额初始化
                            current_amount = float(currency["totalAmount"])
                            value["last_amount"] = current_amount
                            delta_amount = 0
                            logging.warning(
                                f"Redis unavailable, using current amount as initial: {current_amount}"
                            )
                    ...
                    # 存储到Redis
                    try:
                        success = self.last_amount_storage.set_last_amount(
                            symbol,
                            self.symbols[symbol]["last_amount"],
                        ...
                        if not success:
                            logging.error(
                                f"Failed to store last_amount to Redis for {symbol}"
                            )
                    except Exception as e:
                        logging.error(f"Error storing last_amount for {symbol}: {e}")
        return True
    ...
    def run(self):
        while True:
            ...
            res = self.client.futures_change_leverage(
                symbol=self.config["bnsymbol"], leverage=self.config["leverage"]
            )
            logging.info(res)
            ...
            for symbol in self.xt_clients.symbols:
                ...
                delta_position = self.xt_clients.symbols[symbol][
                    "delta_position"
                ] * int(symbol.split("_")[0][-2])
                ...
                delta_amount_heding = self.xt_clients.symbols[symbol][
                    "delta_amount"
                ] * int(symbol.split("_")[0][-2])
                ...
                symbol_delta_amount_heding += (
                    symbol + " " + str(delta_amount_heding) + " "
                )
                symbol_price += symbol + " " + str(round(mid_price, 6)) + " "

                # for comparasion hedging
                amount = (
                    self.xt_clients.symbols[symbol]["last_amount"]
                    - self.xt_clients.symbols[symbol]["init_amount"]
                )
                mid_price = self.xt_clients.symbols[symbol]["mid_price"]
            ...
            price = round(
                float(
                    self.client.futures_symbol_ticker(symbol=config["bnsymbol"])[
                        "price"
                    ]
                ),
                config["precision_price"],
            )

            bn_position = price * bn_coin_remain

            if xt_delta_amount == 0 and abs(xt_position + bn_position) > 0.05 * abs(
                bn_position
            ):
                logging.info(
                    "xt_delta_amount == 0 and xt_position + bn_posiiton > 0.01 * bn_posiiton"
                )
                ...
                message += (
                    "【净敞口】" + str(round(xt_position + bn_position, 2)) + "\n"
                )
            ...
            if abs(xt_position + bn_position) <= 0.05 * abs(bn_position):
                logging.info(
                    f"abs({xt_position} + {bn_position}) <= 0.05 * abs({bn_position})"
                )
                continue
            ...
            if xt_delta_position == 0 and self.remain_amount == 0 and open_value == 0:
                logging.info(
                    "xt_delta_position == 0 and self.remain_amount == 0 and open_value == 0"
                )
                continue
            ...
            if config["Enable_hedging"]:
                if amount >= float(f"1e-{self.config['precision_amount']}"):
                    result = self.create_order(
                        self.config["bnsymbol"], side, price, amount
                    )
                    CHAT_ID = "bfcc4b0a-15ce-4775-a958-24d4ad02ad3f"
                    message = "发生一次对冲\n"
                    message += (
